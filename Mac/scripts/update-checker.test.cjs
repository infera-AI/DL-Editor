const assert = require("assert");

const { buildNoReleaseResult, buildUpdateResult, compareVersions, selectDownloadAsset } = require("../src/main/updateChecker.cjs");

const release = {
  tag_name: "v1.2.0",
  html_url: "https://github.com/infera-AI/DL-Editor/releases/tag/v1.2.0",
  name: "DL Studio 1.2.0",
  published_at: "2026-06-26T08:00:00Z",
  assets: [
    {
      name: "DL-Studio-Mac-1.2.0-arm64.pkg",
      browser_download_url: "https://github.com/infera-AI/DL-Editor/releases/download/v1.2.0/arm64.pkg"
    },
    {
      name: "DL-Studio-Mac-1.2.0-x64.pkg",
      browser_download_url: "https://github.com/infera-AI/DL-Editor/releases/download/v1.2.0/x64.pkg"
    },
    {
      name: "DL-Studio-Mac-1.2.0-arm64.dmg",
      browser_download_url: "https://github.com/infera-AI/DL-Editor/releases/download/v1.2.0/arm64.dmg"
    },
    {
      name: "DL-Studio-Mac-1.2.0-x64.dmg",
      browser_download_url: "https://github.com/infera-AI/DL-Editor/releases/download/v1.2.0/x64.dmg"
    },
    {
      name: "DL-Studio-Windows-Setup-1.2.0.exe",
      browser_download_url: "https://github.com/infera-AI/DL-Editor/releases/download/v1.2.0/exe"
    },
    {
      name: "DL-Studio-Windows-Setup-1.2.0.exe.blockmap",
      browser_download_url: "https://github.com/infera-AI/DL-Editor/releases/download/v1.2.0/blockmap"
    }
  ]
};

assert.equal(compareVersions("1.2.0", "1.1.9"), 1);
assert.equal(compareVersions("v1.0.0", "1.0.0"), 0);
assert.equal(compareVersions("1.0.0", "1.0.1"), -1);

assert.equal(selectDownloadAsset(release, "darwin", "arm64").name, "DL-Studio-Mac-1.2.0-arm64.dmg");
assert.equal(selectDownloadAsset(release, "darwin", "x64").name, "DL-Studio-Mac-1.2.0-x64.dmg");
assert.equal(selectDownloadAsset(release, "win32").name, "DL-Studio-Windows-Setup-1.2.0.exe");

const macArmUpdate = buildUpdateResult({
  currentVersion: "1.0.0",
  platform: "darwin",
  arch: "arm64",
  release
});
assert.equal(macArmUpdate.status, "available");
assert.equal(macArmUpdate.latestVersion, "1.2.0");
assert.equal(macArmUpdate.downloadUrl, "https://github.com/infera-AI/DL-Editor/releases/download/v1.2.0/arm64.dmg");

const macIntelUpdate = buildUpdateResult({
  currentVersion: "1.0.0",
  platform: "darwin",
  arch: "x64",
  release
});
assert.equal(macIntelUpdate.status, "available");
assert.equal(macIntelUpdate.downloadUrl, "https://github.com/infera-AI/DL-Editor/releases/download/v1.2.0/x64.dmg");

const current = buildUpdateResult({
  currentVersion: "1.2.0",
  platform: "win32",
  release
});
assert.equal(current.status, "latest");
assert.equal(current.downloadUrl, "");

const noRelease = buildNoReleaseResult({ currentVersion: "1.0.0" });
assert.equal(noRelease.status, "no_release");
assert.equal(noRelease.currentVersion, "1.0.0");
assert.equal(noRelease.releaseUrl, "https://api.infera.cn/api/infera/client/releases/latest");

console.log("Update checker tests passed.");
