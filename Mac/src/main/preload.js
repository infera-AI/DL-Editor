...
contextBridge.exposeInMainWorld("dlEditor", {
  ...
  streamConversation: (payload) => ipcRenderer.invoke("infera:conversation-stream", payload),
  cancelConversationStream: (streamId) => ipcRenderer.invoke("infera:conversation-stream:cancel", streamId),
  onConversationStreamEvent: (streamId, listener) => on(`infera:conversation-stream:event:${streamId}`, listener),
  streamResearchSimulation: (payload) => ipcRenderer.invoke("infera:research-simulation-stream", payload),
  cancelResearchSimulationStream: (streamId) => ipcRenderer.invoke("infera:research-simulation-stream:cancel", streamId),
  onResearchSimulationStreamEvent: (streamId, listener) => on(`infera:research-simulation-stream:event:${streamId}`, listener),
